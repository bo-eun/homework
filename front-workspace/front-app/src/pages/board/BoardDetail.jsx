...
function BoardDetail() {
    ...
    const deleteFileHandle = async (bfId) => {
        if (isFile.current && confirm("파일이 실제로 삭제됩니다. 진행하겠습니까?")) {
            try {
    ...
    const updateBoardHandler = async (frmData) => {
        ...
        if (frmData.file.length > 0) {
            if (isFile.current) {
                if (!confirm("파일을 등록하면 기존파일이 삭제됩니다. 진행하겠습니까?")) {
                    setValue('file', '');
                    return false;
                }
            }
            ...
            <section className='detail contents'>
                <form action="" onSubmit={handleSubmit(updateBoardHandler)}>
                    <div className="sch">
                        ...
                        <button type='button' className='btn btn-outline-secondary' onClick={goList}>목록</button>
                        <button type='button' className='btn btn-outline-danger' onClick={goDelete}>삭제</button>
                    </div>
                    <div className="board">
                            ...
                            <ul className="file-list mt-2">
                                {
                                    data?.fileList?.map(file =>
                                        <li key={file.bfId}>
                                            {file.fileName}<a href="#" onClick={() => deleteFileHandle(file.bfId)}><RiDeleteBinLine /></a>
                                        </li>
                                    )
                                }
                            </ul>
