  ...
  const renderPage = () => {
    ...
    pageHTML.push(
      <li key="first" className={`page-item ${isDisabled ? "disabled" : ""}`}>
        <a className="page-link" href="#" onClick={(e) => movePage(0)}>
          처음
        </a>
      </li>
    );
    ...
    pageHTML.push(
      <li key="prev" className={`page-item`}>
        <a className="page-link" href="#" onClick={() => movePage(firstPage)}>
          이전
        </a>
      </li>
    );
    ...
    for (let i = 0; i < blockPerCount; i++) {
      ...
      pageHTML.push(
        <li key={pageNum} className={`page-item ${isActive}`}>
          <a className="page-link" href="#" onClick={() => movePage(pageNum)}>
            {pageNum + 1}
          </a>
        </li>
      );
    ...
    pageHTML.push(
      <li key="last" className={`page-item ${isDisabled ? "disabled" : ""}`}>
        <a className="page-link" href="#" onClick={() => movePage(lastPageNum)}>
          마지막
        </a>
      </li>
    );
  ...
  return (
    <>
      <nav aria-label="Page navigation">
        <ul className="pagination justify-content-center">{renderPage()}</ul>
      </nav>
    </>
  );
}
