...
public class ImageReadTest02 {
	public static void main(String[] args) {
		...
		try(FileInputStream in = new FileInputStream(file);) {
			int available = in.available(); // 기다리지 않고 읽을 수 있는 크기
			
			// 블럭킹 당하지 않을 크기가 1kb(1024)보다 크면 그걸쓰고 아니면 1kb로 배열을 만든다.
			int size = available > 1024 ? available : 1024; 
			
			// buffered 사용하면 속도가 좀 더 빠르긴 함... 하지만
			// 배열로 담는게 buffered로 담는것보다 cpu사용량이 적어서 좋다...
			// 제일 좋은 방법은 buffered(보조스트림) + 배열을 섞어 쓰는게 좋다...
